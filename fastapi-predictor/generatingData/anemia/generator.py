...
# --- DataLoader dla VAE (z przeniesieniem danych na device) ---
data_tensor = torch.tensor(df_encoded.values, dtype=torch.float32).to(device)  # <-- zmiana
...
def loss_fn(recon, x, mu, logvar):
    ...
    bce = nn.BCEWithLogitsLoss(reduction='sum')(recon[:, label_start_idx:], x[:, label_start_idx:])  # <-- zmiana (reduction)
...
for epoch in range(1, epochs + 1):
    total = 0
    for batch in loader:
        x = batch[0]  # <-- dane już są na device
        opt.zero_grad()
...
with torch.no_grad():
    ...
    gen_tensor = model.decode(z).cpu().detach()
...
data_save_file_name = os.path.join(BASE_DIR, "data", "anemia", "dane-anemia.csv")

df_generated.to_csv(data_save_file_name, index=False)
